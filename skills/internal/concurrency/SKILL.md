---
name: concurrency
description: Use when applying the staged concurrency procedure. Apply the narrow concurrency procedure and return evidence, exceptions and a bounded recommendation.
---
...
Apply the narrow concurrency procedure and return evidence, exceptions and a bounded recommendation.

## Routing contract

- Positive: Apply the narrow concurrency procedure and return evidence, exceptions and a bounded recommendation.
- Negative: Do not trigger for a neighbouring capability merely sharing words with concurrency.
...
- Positive case must select `engineering/concurrency` and produce evidence.
