...
describe('remote web preview guard', () => {
  it.each([
    ...
    'https://[::1]:5173',
    'https://site.local',
  ])('blocks local browser target %s', (url) => {
  ...
  it('accepts remote Cloudflare preview URLs', () => {
    expect(
      assertRemotePreviewUrl('https://atelier-v1-chuka-personal-site.account.workers.dev').hostname,
    ).toContain('workers.dev');
    expect(assertRemotePreviewUrl('https://chuka-personal-site.pages.dev').hostname).toContain(
      'pages.dev',
    );
  });
});
