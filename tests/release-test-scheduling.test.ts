...
import {
  isolatedResourceTestFiles,
  resourceTestFiles,
  sharedResourceTestFiles,
} from '../vitest.resource-test-files.js';
...
describe('release test scheduling', () => {
  it('keeps ordinary tests parallel and serializes only shared host-resource tests', () => {
    ...
    expect(packageJson.scripts.test).toBe(
      'pnpm test:parallel && pnpm test:resource && pnpm test:isolated-resource',
    );
    ...
    expect(resourceConfig).toContain('include: sharedResourceTestFiles');
    expect(resourceConfig).toContain('maxWorkers: 1');
    expect(resourceConfig).toContain('fileParallelism: false');
    expect(sharedResourceTestFiles).toEqual([
    ...
    expect(isolatedResourceTestFiles).toEqual([
      'tests/cli.test.ts',
      'tests/skill-host-commands.test.ts',
    ]);
    expect(resourceTestFiles).toEqual([...sharedResourceTestFiles, ...isolatedResourceTestFiles]);
  });
});
