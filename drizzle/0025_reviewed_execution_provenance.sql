ALTER TABLE independent_review_receipts ADD COLUMN reviewed_run_id text REFERENCES agent_runs(id);
--> statement-breakpoint
ALTER TABLE independent_review_receipts ADD COLUMN source_tree_digest text;
--> statement-breakpoint
...
BEGIN
  ...
  THEN RAISE(ABORT, 'independent review receipt requires distinct canonical succeeded reviewed and review runs') END;
END;
--> statement-breakpoint
...
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    ...
    WHERE irr.id = NEW.receipt_id AND irr.project = NEW.project AND irr.goal_id = NEW.goal_id
      ...
      AND reviewed.source_head = NEW.source_head AND reviewed.status = 'succeeded'
  ) THEN RAISE(ABORT, 'supervisor completion commit requires execution-bound canonical review authority') END;
END;
--> statement-breakpoint
CREATE TRIGGER independent_review_receipts_reviewed_run_immutable
BEFORE UPDATE OF reviewed_run_id, source_tree_digest ON independent_review_receipts
BEGIN
...
BEGIN
  ...
  ) THEN RAISE(ABORT, 'independent task completion requires Major-owned execution-bound review provenance') END;
END;
