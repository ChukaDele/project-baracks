...
/** Extend an active claim's lease. Throws StaleClaimError when the claim is
 * no longer this worker's active claim (expired and recovered, cancelled…). */
export function heartbeatClaim(
  ...
  const result = db
    ...
    .run();
  if (result.changes !== 1) {
    throw new StaleClaimError(`claim ${claimId} is not an active claim of worker ${workerId}`);
  }
...
export function completeClaim(db: Db, claimId: string, workerId: string, reason = 'completed') {
  return db.transaction((tx) => closeClaim(tx, claimId, workerId, 'completed', reason), {
    behavior: 'immediate',
  });
}
...
export function releaseClaim(
  db: Db,
  options: { claimId: string; workerId: string; requeue: boolean; reason: string },
) {
  return db.transaction(
    (tx) => {
      const claim = closeClaim(tx, options.claimId, options.workerId, 'released', options.reason);
